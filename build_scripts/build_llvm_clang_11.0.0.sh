...
echo "THIS SCRIPT IS WORK IN PROGRESS -- DOES NOT YET WORK"
exit 1

# Instructions for building llvm & clang 11.0.0 from source.
...
# its location in 'gcc_custom_prefix_dir' variable. Or comment this out to use
...
# Starting with LLVM 12.0.0 we need to have cmake >= 3.13.4.
CMAKE=$(which cmake)
...
# TODO: uncomment these additional projects, and try to add others.

llvm_tarfile=llvm-${llvm_version}.src.tar.xz
clang_tarfile=clang-${llvm_version}.src.tar.xz
...
for f in ${llvm_tarfile} ${clang_tarfile} ${compiler_rt_tarfile} \
         ...
         ${libunwind_tarfile} ;
do
    # Note: for llvm 11.0.0, the source files are hosted on github.
    __wget https://github.com/llvm/llvm-project/releases/download/llvmorg-${llvm_version}   $f
done
